...
func init() {
	hydra.Conf.Custom(dns.DDNS, conf.New(conf.WithDisable(), conf.WithTimeout(10, 10))).
		Sub(conf.TypeNodeName, conf.NewNames("114.114.114.114", "8.8.8.8"))
	dns.App.Micro("/ddns", services.NewDdnsHandler())
	dns.App.Micro("/github/ip/request", services.NewGithubHandler())
	dns.App.CRON("/github/ip/request", services.NewGithubHandler(), "@midnight")
	hydra.OnReady(func() {
